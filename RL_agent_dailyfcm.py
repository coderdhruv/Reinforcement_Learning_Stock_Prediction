...
from preprocess import preprocess
from fcm import FCmeans, FCmeansDaily
...
def sliding_window_three_days(df, date, training_data):
    ...
    starting_date_obj = datetime.datetime.strptime(
        date, "%Y-%m-%d")
    fcmeansDaily = FCmeansDaily(training_data)
    ...
    clustercentres = fcmeansDaily.get_clusters()
    index_of_first_cluster_center = fcmeansDaily.get_labels_for_each_data_point(
        index_of_starting_data)
    index_of_second_cluster_center = fcmeansDaily.get_labels_for_each_data_point(
        index_of_starting_data + 1)
    index_of_third_cluster_center = fcmeansDaily.get_labels_for_each_data_point(
